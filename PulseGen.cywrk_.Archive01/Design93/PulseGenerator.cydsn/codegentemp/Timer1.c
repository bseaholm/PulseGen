...
uint32 Timer1_ReadPeriod(void) 
{
   ...
   #else
       return (CY_GET_REG32(Timer1_PERIOD_LSB_PTR));
   #endif /* (Timer1_UsingFixedFunction) */
}
...
void Timer1_WritePeriod(uint32 period) 
{
    ...
    #else
        CY_SET_REG32(Timer1_PERIOD_LSB_PTR, period);
...
uint32 Timer1_ReadCapture(void) 
{
   ...
   #else
       return (CY_GET_REG32(Timer1_CAPTURE_LSB_PTR));
   #endif /* (Timer1_UsingFixedFunction) */
}
...
void Timer1_WriteCounter(uint32 counter) 
{
    ...
    #else
        CY_SET_REG32(Timer1_COUNTER_LSB_PTR, counter);
